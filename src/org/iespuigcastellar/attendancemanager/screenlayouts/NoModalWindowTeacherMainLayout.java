...
import org.iespuigcastellar.attendancemanager.core.Group;
...
import org.iespuigcastellar.attendancemanager.core.Subject;
...
public class NoModalWindowTeacherMainLayout extends VerticalLayout implements Property.ValueChangeListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -2615947020726259241L;
	...
	public NoModalWindowTeacherMainLayout() {		
    ...
    public void changedClassblock(ValueChangeEvent event) {
    	...
    	if(classblock != null) {
    		...
    		getWindow().showNotification("Select a valid classblock", "Please select a date and a valid classblock", Notification.TYPE_HUMANIZED_MESSAGE);
    	}
    }
    ...
    private class PasswordChangeLayout extends VerticalLayout {
    	...
    	public PasswordChangeLayout() {
    		
    		changePasswordButton = new Button(app.locale.getString("TEACHERMAINLAYOUT_CHANGEMYPASSWORD_BUTTON"));
    		...
    		changePasswordButton.addListener(new Button.ClickListener() {
				public void buttonClick(ClickEvent event) {
					...
					passwordForm.getField("passwordA").focus();
				}
			});
    		
    		HorizontalLayout labelHorizontalLayout = new HorizontalLayout();
    		labelHorizontalLayout.addComponent(changePasswordButton);
    		labelHorizontalLayout.setComponentAlignment(changePasswordButton, Alignment.MIDDLE_LEFT);
    		addComponent(labelHorizontalLayout);
    		...
    		okButton.addListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if(passwordInfo.passwordA.equals(passwordInfo.passwordB)) {
						...
						getWindow().showNotification(app.locale.getString("TEACHERMAINLAYOUT_PASSWORDCHANGEOKNOTIFICATION_CAPTION"), app.locale.getString("TEACHERMAINLAYOUT_PASSWORDCHANGEOKNOTIFICATION_DESCRIPTION"), Notification.TYPE_TRAY_NOTIFICATION);
						changePasswordButton.setVisible(true);
						passwordForm.setVisible(false);
						passwordForm.getField("passwordA").setValue("");
						passwordForm.getField("passwordB").setValue("");
					} else {
						getWindow().showNotification(app.locale.getString("TEACHERMAINLAYOUT_PASSWORDCHANGEERRORNOTIFICATION_CAPTION"), app.locale.getString("TEACHERMAINLAYOUT_PASSWORDCHANGEERRORNOTIFICATION_DESCRIPTION"), Notification.TYPE_WARNING_MESSAGE);
					}
